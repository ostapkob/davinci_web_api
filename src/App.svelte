...
export let title, defaultModel, models, temperature, maxTokens, apiKey, prompt
...
async function getResponse() {
    ...
    try {
        ...
        if (response?.data?.choices?.length) {
            ...
            answer = response.data.choices.map(choice => choice.text).join("\n");
            saveToHistory();
...
<main>
    <div class="title" span={12}>
        ...
        <div style="display: flex; align-items: center;">
            <IconButton class="material-icons" on:click={() =>changeShowKey() }>key</IconButton >
        </div>
        <div style="display: flex; align-items: center;">
            <IconButton class="material-icons" on:click={() =>changeShowHistory() }>history</IconButton >
        </div>
    </div>
    {#if showKey}
    ...
    <div id="form">
        <LayoutGrid>
            ...
            <Cell  span={12}>
                <!-- <div class="margins" style="margin: 10px;" > -->
                <div >
                    <Button on:click={getResponse} id="generateBtn" variant="raised" disabled={isLoading}>{isLoading ? 'Loading...' : 'Get Response'}</Button>

                    {#if isLoading}
                    <Button on:click={cancelRequest} id="generateBtn" color="secondary" variant="raised" >{'Cancel'}</Button>
                    {/if}
                </div>
...
<Button on:click={saveToHistory}  variant="raised" >{'History'}</Button>
    </main>
